...
function buildWorkOrder(input, config) {
  ...
  const definitions = (config.checklist || []).filter(s => s && s.include !== false && Array.isArray(s.appliesTo) && s.appliesTo.some(t => types.includes(t)));
  ...
  const log = { id, assignmentId: id, event: 'assignment_report_created', createdAt, tmv, technician: name, location, date };
